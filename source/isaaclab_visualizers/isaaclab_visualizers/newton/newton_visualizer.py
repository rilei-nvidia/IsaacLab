...
def _ovrtx_rtx_soname_fixed() -> bool:
    """Return ``True`` if OVRTX's ``librtx.hydra.so`` has a unique SONAME (patchelf fix applied).

    Without the fix, Kit's copy of ``librtx.hydra.so`` (same SONAME, loaded globally by carb)
    shadows OVRTX's copy, breaking ``rtxHydraSetFramework`` resolution.  Fix with::

        .venv/bin/patchelf --set-soname libovrtx.rtx.hydra.so \\
            <ovrtx>/bin/plugins/rtx/librtx.hydra.so
    ...
    try:
        ...
        if spec is None:
            return False
        lib = pathlib.Path(spec.origin).parent / "bin" / "plugins" / "rtx" / "librtx.hydra.so"
        if not lib.exists():
            return False
        patchelf = pathlib.Path(".venv/bin/patchelf")
        if not patchelf.exists():
            return False
        result = subprocess.run([str(patchelf), "--print-soname", str(lib)], capture_output=True, text=True)
        return result.returncode == 0 and result.stdout.strip() != "librtx.hydra.so"
    except Exception:
        return False
...
class _NewtonViewerUIMixin:
    ...
    def _render_training_controls(self, imgui):
        ...
        self._draw_streaming_view_controls()
    ...
    def _draw_streaming_view_controls(self) -> None:
        ...
        imgui = self.ui.imgui
        if not imgui.collapsing_header("Streaming View", imgui.TreeNodeFlags_.default_open.value):
            return
...
class NewtonVisualizer(BaseVisualizer):
    ...
    def supports_live_plots(self) -> bool:
        """Newton viewers do not currently provide live-plot panels."""
        return False
...
class NewtonGLVisualizer(NewtonVisualizer):
    ...
    def _log_streaming_image(self) -> None:
        ...
        self._viewer.log_image("Streaming View", wp.from_torch(composite_t))
...
class NewtonRTXVisualizer(NewtonVisualizer):
    ...
    def _create_viewer(self, runtime_headless: bool, metadata: dict) -> NewtonViewerRTX:
        # Guard: if Kit has already loaded its own librtx.hydra.so with RTLD_GLOBAL,
        # OVRTX's copy of the same SONAME won't be found, causing a symbol lookup error
        # for rtxHydraSetFramework. Detect this by checking whether OVRTX's copy has had
        # its SONAME renamed via patchelf (the permanent fix). If not patched, warn and skip.
        #   Fix: patchelf --set-soname libovrtx.rtx.hydra.so \
        #            .../ovrtx/bin/plugins/rtx/librtx.hydra.so
        #        patchelf --replace-needed librtx.hydra.so libovrtx.rtx.hydra.so \
        #            .../ovrtx/bin/plugins/rtx/libovrtx.dylib.so
        _soname_fixed = _ovrtx_rtx_soname_fixed()
        if not _soname_fixed:
            logger.warning(
                "[NewtonRTXVisualizer] OVRTX's librtx.hydra.so SONAME conflicts with Kit's copy. "
                "Newton RTX viewer will be skipped to avoid a process crash. "
                "Apply the patchelf SONAME rename to fix permanently:\n"
                "  .venv/bin/patchelf --set-soname libovrtx.rtx.hydra.so "
                "<ovrtx>/bin/plugins/rtx/librtx.hydra.so\n"
                "  .venv/bin/patchelf --replace-needed librtx.hydra.so libovrtx.rtx.hydra.so "
                "<ovrtx>/bin/plugins/rtx/libovrtx.dylib.so"
    ...
    def _pump_paused(self) -> None:
        # Must call both begin_frame AND end_frame to close the imgui frame.
        # When _paused=True, log_state() is not called so the OVRTX scene is
        # frozen — the path-tracer keeps accumulating samples on the static scene,
        # which means the image converges/clears while paused. This is correct
        # path-tracer behavior: the longer you pause, the cleaner the image.
        self._viewer.begin_frame(self._sim_time)
        self._viewer.end_frame()

    def _uses_streaming_view(self) -> bool:
        # Streaming view uses a TiledCamera sensor independent of ViewerRTX, so it
        # is fully supported even though render_rgb_array() (interactive frame capture)
        # is still stubbed pending ViewerRTX.get_frame().
        return bool(self.cfg.streaming_view)

    def _log_streaming_image(self) -> None:
        """Push the composited streaming frame into Newton's image panel."""
        if self._viewer is None or self._camera_sensor is None:
            return
        if self._camera_is_owned:
            self._update_owned_camera_poses()
