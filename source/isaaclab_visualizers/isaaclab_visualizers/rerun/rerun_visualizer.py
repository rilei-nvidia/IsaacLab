...
def _resolve_streaming_renderer_cfg(renderer_name: str | None):
    ...
    if renderer_name == "ovrtx":
        try:
            ...
            # OVRTX requires exclusive ownership of the /Render prim.  If it already
            # exists (e.g. another OVRTX renderer is running), fall back to newton_warp.
            try:
                from pxr import Usd
                ...
                if stage is not None and stage.GetPrimAtPath("/Render").IsValid():
                    logger.info(
                        "[RerunVisualizer] /Render prim already exists; "
            ...
            return NewtonWarpRendererCfg()
    raise ValueError(f"streaming_cam_renderer={renderer_name!r} unsupported. Use 'newton_warp', 'ovrtx', or None.")
...
class NewtonViewerRerun(ViewerRerun):
    ...
    def _get_blueprint(self):
        ...
        When streaming is active, the streaming composite (``Spatial2DView``) is
        the primary full-width panel.  Live-plot time-series views are appended
        as a narrow right column when registered.  The 3D Newton view is shown
        as a small secondary panel below the stream so physics state is still
        visible but not dominant.

        The stored :attr:`_camera_pose` is forwarded to
        :class:`~rerun.blueprint.EyeControls3D` when the 3D view is included.
        """
        # Streaming composite fills the full viewport; the 3D Newton view is omitted.
