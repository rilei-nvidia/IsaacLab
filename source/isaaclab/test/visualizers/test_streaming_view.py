...
def test_layout_single_gt_square_grid():
    # 16 envs × 1 GT, square frame → should pick 4 cols (4×4 grid)
...
# ---------------------------------------------------------------------------
# Newton RTX OVRTX fix detection
# ---------------------------------------------------------------------------


def test_ovrtx_soname_fixed_returns_bool():
    """_ovrtx_rtx_soname_fixed() returns a bool regardless of patchelf presence."""
    from isaaclab_visualizers.newton.newton_visualizer import _ovrtx_rtx_soname_fixed

    result = _ovrtx_rtx_soname_fixed()
    assert isinstance(result, bool)


def test_ovrtx_soname_fixed_detects_rename():
    """If the patchelf rename was applied, the function returns True."""
    ...
    if spec is None:
        return  # ovrtx not installed — skip
    ...
    if not patchelf.exists():
        return  # patchelf not installed — skip
    result = subprocess.run([str(patchelf), "--print-soname", str(lib)], capture_output=True, text=True)
    if result.returncode != 0:
        return
    soname_was_renamed = result.stdout.strip() != "librtx.hydra.so"

    from isaaclab_visualizers.newton.newton_visualizer import _ovrtx_rtx_soname_fixed

    assert _ovrtx_rtx_soname_fixed() == soname_was_renamed
